...
begin
  ...
  if StringLength > 0 then begin
    ...
    for Position := 1 to StringLength do begin
      StringChar := AnsiChar(FunctionName[Position]);
      if ASSIGNED(Node) then begin
        ...
        for PositionCounter := Position to StringLength do begin
          NewNode := CreateExportTreeNode(AnsiChar(FunctionName[PositionCounter]));
...
begin
  ...
  if RESULT < 0 then begin
    ...
    ExternalLibraryArray[RESULT].LibraryHandle := LoadLibraryA(PANSICHAR(LibraryName));
  end;
end;
        ...
        while ImportDescriptor^.Name <> 0 do begin
          ...
          while ThunkData^ <> 0 do begin
            ...
            if (ThunkData^ and IMAGE_ORDINAL_FLAG32) <> 0 then begin
              ...
              FunctionPointer := GetProcAddress(DLLImport^.LibraryHandle, PANSICHAR(ThunkData^ and IMAGE_ORDINAL_MASK32));
            end
            else begin
        ...
        for I := 0 to ExportDirectory^.NumberOfNames - 1 do begin
          ...
          if (LONGWORD(ExportDirectory) < LONGWORD(FunctionPointer)) and (LONGWORD(FunctionPointer) <
            ...
            if ForwarderCharPointer^ = '#' then begin
              ...
              ForwarderString := ForwarderCharPointer;
              ExportArray[I].FunctionPointer := GetProcAddress(ForwarderLibraryHandle, PAnsiChar(ForwarderString));
            end;
          end
          else begin
