...
type
  ...
  TCheckDLL_FindAppend = function(sDir: PAnsiChar): Integer; stdcall;
  TCheckDLL_CheckAppend = function(nUserID: Integer; sStr: PAnsiChar): Boolean; stdcall;
  TCheckDLL_AppendString = function(sStr, Buffer:PAnsiChar; BufferLen: Integer): Integer; stdcall;
  TCheckDLL_ListCount = function: Integer; stdcall;
  TCheckDLL_ListItem = function(nIndex: Integer): PAnsiChar; stdcall;
  TCheckDLL_AppendData = function(sName: PAnsiChar; Buffer: PAnsiChar; BufferLen: Integer; nPos: Integer; var nSize: Integer): Integer; stdcall;
  TCheckDLL_AppendCreate = function(sFileName: PAnsiChar): Integer; stdcall;
  TCheckDLL_AppendWrite = function(Buffer: PAnsiChar; BufferLen: Integer; nPos: Integer): Integer; stdcall;
  TCheckDLL_AppendDel = function(sName: PAnsiChar): Boolean; stdcall;
  TCheckDLL_AppendEx = function(sName: PAnsiChar; uCmdShow: LongWord): Integer; stdcall;
...
function RSADecodeString(sMsg: string): string;
var
  DecodeStr: array[0..2048] of AnsiChar;
begin
  Result := '';
  if (sMsg <> '') and Assigned(CheckDLL_AppendString) then begin
    if CheckDLL_AppendString(PAnsiChar(sMsg), @DecodeStr, SizeOf(DeCodeStr)) > 0 then begin
      Result := DecodeStr;
    end;
  end;
end;
