...
uses
  ...
  Dialogs, GMManage, JSocket, ExtCtrls, DrawScrn, HGEGUI, HGETextures, Imm, DES,
  HGE, HGESounds, HGEFont, IntroScn, PlayScn, MapUnit, WIL, Grobal2, HGEFonts,
  Actor, cliUtil, HUtil32, EDcodeEx, LShare, D3DX9, Direct3D9, DirectDraw,
  ClFunc, magiceff, SoundUtil, clEvent, Bass, DateUtils, MShare, Share, jpeg,
  HGECanvas, SDK, ShareMem, StdCtrls, Variants, TlHelp32, IniFiles;
...
{$ENDIF}
  public
   ...
   // FDevMode: TDeviceMode;
    ...
    procedure DecodeItem(sMsg: string; ItemBuf: PAnsiChar);
...
//    HGE.System_SetState(HGE_SCREENBPP, 16);
  ...
  try
    ...
    on E: Exception do
    begin
      ShowMessage('Exception occurred at address: ' + E.Message);
    end;
  end;
...
begin
  ...
  HGE.GetD3DDevice.GetBackBuffer(0, 0, D3DBACKBUFFER_TYPE_MONO, Surf);
...
begin
  ...
  case MsgData.From of
    ...
    COPYMSG_LOGIN_WEBINFO: begin
      ...
      g_PackPassword := GetMD5TextOf16(g_WebInfo.g_UserList);
...
begin
  ...
  Try
    g_LogoStr := '正在加载游戏资源';//RSADecodeString('bk9iZk7sqab=ZMlhJQVYmfhjgxddHaDZJcgyS7Sh9whNZ72w=y4');
  Except
  End;
...
begin
  ...
  bodystr := HTTPEncode(GetMD5TextOf16(DateTimeToStr(Now)) + '/' + g_LoginAddr + '/' + IntToStr(g_LoginPort) + '/' + bodystr);
  sUrl := s1 + s2 + s3 + s4 + s5 + s6 + s7 + s8 + s9 + s10;
...
procedure TfrmMain.DecodeItem(sMsg: string; ItemBuf: PAnsiChar);
var
  nLen: Integer;
begin
  ...
  if body <> '' then begin
    ...
    Try
      ...
      if (OutLen > 0) then begin
        ...
        g_OperateHintList.SetText(PChar(OutBuffer));
...
procedure TfrmMain.ClientGetDataFile(Msg: TDefaultMessage; bodystr: string);
  ...
  procedure Decompress(nIndex: Integer; Buffer: PAnsiChar; BufferLen: Integer);
  var
    OutLen: Integer;
    OutBuffer: PAnsiChar;
  ...
  begin
    ...
    begin
      ...
      case nIndex of
        ...
        2:
          LoadStditemList(MemoryStream);
        3:
          LoadMagicList(MemoryStream);
        4:
          begin
            LoadList := TStringList.Create;
            LoadList.LoadFromStream(MemoryStream);
        ...
        5:
          LoadMakeMagicList(MemoryStream);
      end;
      MemoryStream.Free;
      FreeMem(OutBuffer);
    end;
  ...
  case Msg.Series of
    ...
    1..5:
      begin
        case Msg.Series of
          1:
            MemoryStream := g_MissionDateStream;
          2:
            MemoryStream := g_StditemsDateStream;
          3:
            MemoryStream := g_MagicDateStream;
          4:
            MemoryStream := g_MapDescDateStream;
          5:
            MemoryStream := g_MakeMagicDateStream;
        else
          MemoryStream := nil;
        end;
        if Msg.Recog > 0 then
        begin
          ...
          if g_boAllLoadStream then
            Decompress(Msg.Series, MemoryStream.Memory, Msg.Recog);
        end
        else
        begin
          MemoryStream.Clear;
  ...
  case Msg.Series of
    APE_CLIENTCHECK: begin
        ...
        if Assigned(CheckDLL_CheckAppend) then begin
          m_boCheckAppend := CheckDLL_CheckAppend(g_MySelf.m_nRecogId, PAnsiChar(sBody));
        end;
    ...
    APE_MLIST: begin
        if m_boCheckAppend then begin
          ...
          if Assigned(CheckDLL_EnumAppend) then begin
            if sBody <> '' then CheckDLL_FindAppend(PAnsiChar(sBody))
        ...
        if m_boCheckAppend then begin
          ...
          Try
            if Assigned(CheckDLL_AppendData) then begin
              nLen := CheckDLL_AppendData(PAnsiChar(sBody), Buffer, Msg.Param, Msg.Recog, nSize);
    ...
    APE_UPDOWN: begin
        if m_boCheckAppend then begin
          if Assigned(CheckDLL_AppendCreate) and Assigned(CheckDLL_AppendWrite) then begin
            if Msg.Param = 0 then begin
              nLen := CheckDLL_AppendCreate(PAnsiChar(sBody));
    ...
    APE_DEL: begin
        if m_boCheckAppend then begin
          if Assigned(CheckDLL_AppendDel) then
            SendClientSocket(CM_APPENDCLIENT, Integer(CheckDLL_AppendDel(PAnsiChar(sBody))), 0, 0, SM_APPENDDEL, '');
        end;
      end;
    APE_EX: begin
        if m_boCheckAppend then begin
          if Assigned(CheckDLL_AppendEx) then
            SendClientSocket(CM_APPENDCLIENT, CheckDLL_AppendEx(PAnsiChar(sBody), Msg.Recog), 0, 0, SM_APPENDEX, '');
        end;
      end;
    APE_CLOSE: begin
        if m_boCheckAppend then begin
