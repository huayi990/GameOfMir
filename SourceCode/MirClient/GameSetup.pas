  ...
  begin
    ...
    try
      for i := Low(g_CustomKey) to High(g_CustomKey) do begin
        Ini.WriteInteger(ShiftName, g_CustomKey[i].KeyHint, (PInteger(@g_CustomKey[i].CustomKey.Shift))^);
        Ini.WriteInteger(KeyName, g_CustomKey[i].KeyHint, g_CustomKey[i].CustomKey.Key);
      end;
    finally
      Ini.Free;
    end;
...
procedure LoadIDInfo();
  ...
  begin
    if FileExists(sFileName) then begin
      ...
      try
        for i := Low(g_CustomKey) to High(g_CustomKey) do begin
          g_CustomKey[i].CustomKey.Shift := TShiftState((Pointer(Ini.ReadInteger(ShiftName, g_CustomKey[i].KeyHint, 0)))^);
          g_CustomKey[i].CustomKey.Key := Ini.ReadInteger(KeyName, g_CustomKey[i].KeyHint, 0);
        end;
      finally
        Ini.Free;
      end;
