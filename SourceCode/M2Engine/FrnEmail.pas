...
type
  ...
  TEMailInfo = packed record
    ...
    Item: TUserItem;
    TextLen: Word;
    Text: array[0..MAXEMAILTEXTLEN - 1] of AnsiChar;
  end;
  ...
  protected
    ...
    function WriteData(nIndex: Integer; Buffer: PAnsiChar; nLen: Integer): Integer;
    function ReadData(nIndex: Integer; Buffer: PAnsiChar; nLen: Integer): Integer;
  ...
  Try
    for I := 0 to FNameList.Count - 1 do begin
      ...
      if sName <> SYSEMAILNAME then begin
        ...
        nIndex := WriteData(nIDIndex, PAnsiChar(EMailInfo), SizeOf(TEMailInfo));
        ...
        Try
          while True do begin
            if FFileStream.Read(OldEMailInfo, SizeOf(OldEMailInfo)) = SizeOf(OldEMailInfo) then begin
              ...
              Move(PAnsiChar(AnsiString(EMailInfo.Text))^, EMailInfo.Text[0], SizeOf(OldEMailInfo.Text));
  ...
  Try
    ...
    if sMsg <> '' then begin
      try
        ...
        GetMem(SendMessage.Buff, Length(AnsiString(sMsg)) + 1);
        Move(PAnsiChar(AnsiString(sMsg))^, SendMessage.Buff^, Length(AnsiString(sMsg)) + 1);
      except
...
begin
  ...
  Move(PAnsiChar(AnsiString(AText))^, EMailInfo.Text[0], EMailInfo.TextLen);
  if AAll then
      ...
      for I := EMailUserInfo.EMailList.Count - 1 downto 0 do begin
        ...
        if nIndex > -1 then begin
          if EMailListInfo.nIndex = nIndex then begin
            WriteData(EMailListInfo.nIndex, PAnsiChar(@boDelete), 1);
  ...
  Try
    ...
    if nIDIndex <> -1 then begin
      ...
      for I := 0 to EMailUserInfo.EMailList.Count - 1 do begin
        EMailListInfo := EMailUserInfo.EMailList[I];
        if EMailListInfo.nIndex = nIndex then begin
          if ReadData(nIndex, PAnsiChar(@EMailInfo), SizeOf(EMailInfo)) = SizeOf(EMailInfo) then begin
            if EMailInfo.Item.MakeIndex <> 0 then begin
              ...
              End;
              if EMailInfo.Item.MakeIndex = 0 then begin
                WriteData(nIndex, PAnsiChar(@EMailInfo), SizeOf(EMailInfo));
                exit;
              end;
            end;
          end;
      ...
      for I := 0 to EMailUserInfo.EMailList.Count - 1 do begin
        ...
        if EMailListInfo.nIndex = nIndex then begin
          if ReadData(nIndex, PAnsiChar(@EMailInfo), SizeOf(EMailInfo)) = SizeOf(EMailInfo) then begin
            ...
            WriteData(nIndex, PAnsiChar(@EMailInfo), 2);
...
function TUserEMail.WriteData(nIndex: Integer; Buffer: PAnsiChar; nLen: Integer): Integer;
begin
  Result := 0;
...
function TUserEMail.ReadData(nIndex: Integer; Buffer: PAnsiChar; nLen: Integer): Integer;
begin
  Result := 0;
  if nIndex < 0 then exit;
  FFileStream.Seek(nIndex * SizeOf(TEMailInfo), soFromBeginning);
