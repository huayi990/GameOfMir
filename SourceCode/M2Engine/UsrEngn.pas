...
interface
uses
  Windows, Classes, SysUtils, StrUtils, Forms, ObjBase, ObjPlay, ObjNpc, Envir, Grobal2, SDK;
type
...
{$IFEND}
  ...
  public
    ...
    procedure ProcessUserMessage(PlayObject: TPlayObject; DefMsg:
      pTDefaultMessage; Buff: PAnsiChar);
...
{$IF Public_Ver = Public_Release}
  ...
  procedure RegDll_OutMessage(Msg: PAnsiChar; nMode: Integer); stdcall;
...
begin
  ...
  g_ADNoticeList.SetText(PAnsiChar(RegDll_TextList()));
  g_ADExitUrlList.SetText(PAnsiChar(RegDll_Address()));
  g_ADFrameUrlList.SetText(PAnsiChar(RegDll_LogoList()));
