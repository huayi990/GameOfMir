...
begin
  Result := False;
  ;
  sFileName := g_Config.sGameDataDir + 'AdminList.txt';//RSADecodeString('hkZIlTfk5YbIm2j2KwGtziv58WLB9F0foa'); //'AdminList.txt'
...
begin
  Result := -1;
  sFileName := g_Config.sEnvirDir + 'GuardList.txt';//RSADecodeString('hkZIlTfk5YbIm2j2kcqZofTwCByu1LtiIa'); //'GuardList.txt'
...
begin
  ...
  try
    try
      ...
      OutLen := ZIPCompress(PAnsiChar(AnsiString(ClientItemStr)), Length(AnsiString(ClientItemStr)), 9, OutBuffer);
...
begin
  ...
  try
    ...
    OutLen := ZIPCompress(PAnsiChar(AnsiString(ClientStr)), Length(AnsiString(ClientStr)), 9, OutBuffer);
    if OutLen > 0 then begin
      //MainOutmessage(IntToStr(OutLen));
...
function TFrmDB.LoadBoxs(): Integer;
  ...
  begin
    ...
    BoxInfo := nil;
    boReadHeader := False;
    sFileName := g_Config.sEnvirDir + 'box.txt';//RSADecodeString('ba9FoJaLQbWB7qBlqa') + IntToStr(nIndex) + '.txt'; //'box.txt'
    if MyFileExists(sFileName) then begin
...
begin
  ...
  EnterCriticalSection(ProcessHumanCriticalSection);
  try
    sFileName := g_Config.sEnvirDir + 'BoxList.txt';//RSADecodeString('uNAee9FOoy53RI3B'); //'BoxList.txt'
...
begin
  ...
  sFileName := g_Config.sEnvirDir + 'MakeItem.txt';//RSADecodeString('AcIADfibOyWjmNj1Zq'); //'MakeItem.txt'
  if MyFileExists(sFileName) then begin
    LoadList := TMsgStringList.Create;
    LoadList.LoadFromFile(sFileName);
    LoadListCall(ExtractFileName(sFileName), LoadList);
...
begin
  Result := -1;
  sFileName := g_Config.sEnvirDir + 'MapInfo.txt';//RSADecodeString('5hhM2jsC2JwW3vmo'); //'MapInfo.txt'
...
begin
  ...
  sFileName := g_Config.sEnvirDir + 'MapEvent.txt';//RSADecodeString('alOQkQU1xSdRmnO6eq'); //'MapEvent.txt'
...
begin
  Result := 1;
  sFileName := g_Config.sEnvirDir + 'MapQuest.txt';//RSADecodeString('aUDvMYm4cV1U78oDNy'); //'MapQuest.txt'
  if MyFileExists(sFileName) then begin
...
var
  ...
  nFlag: Integer;
begin
  sFileName := g_Config.sEnvirDir + 'Merchant.txt';//RSADecodeString('Az21y67tgkdut8a=Tq'); //'Merchant.txt'
  if MyFileExists(sFileName) then begin
...
begin
  Result := 0;
  sFileName := g_Config.sEnvirDir + 'MiniMap.txt';//RSADecodeString('a2k9Cxbasda3bI2SMa'); //'MiniMap.txt'
  if MyFileExists(sFileName) then begin
    MiniMapList.Clear;
...
begin
  Result := 0;
  sFileName := g_Config.sEnvirDir + 'MonGen.txt';//RSADecodeString('aWEG6Ggm6LYbq58sDy'); //'MonGen.txt'
  if MyFileExists(sFileName) then begin
    LoadList := TMsgStringList.Create;
    LoadList.LoadFromFile(sFileName);
    LoadListCall(ExtractFileName(sFileName), LoadList);
...
begin
  Result := True;
  sFileName := g_Config.sEnvirDir + 'DefiniensConst.txt';//RSADecodeString('m8LG9cipKXSVJRwncVbNh=Pqc+WxLdvsRi'); //'DefiniensConst.txt'
...
var
  ...
  nFlag: Integer;
begin
  sFileName := g_Config.sEnvirDir + 'Npcs.txt';//RSADecodeString('A6TRU9vouO40NMfNiy'); //'Npcs.txt'
  if MyFileExists(sFileName) then begin
    LoadList := TMsgStringList.Create;
    LoadList.LoadFromFile(sFileName);
    LoadListCall(ExtractFileName(sFileName), LoadList);
...
begin
  Result := 0;
  sFileName := g_Config.sEnvirDir + 'StartPoint.txt';//RSADecodeString('Gj2YT8gu=EzNdfTjwtAt3S9AJBN9iRZ7vq'); // 'StartPoint.txt'
  if MyFileExists(sFileName) then begin
    try
...
begin
  Result := 0;
  sFileName := g_Config.sEnvirDir + 'UnbindList.txt';//RSADecodeString('H=5Cb2Cc4kMGJ1TkKHdqEggBAVqWRuoIlq'); //'UnbindList.txt'
  if MyFileExists(sFileName) then begin
    LoadList := TMsgStringList.Create;
    LoadList.LoadFromFile(sFileName);
    LoadListCall(ExtractFileName(sFileName), LoadList);
