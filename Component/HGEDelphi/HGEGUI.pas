...
uses
  ...
  System.Classes, System.SysUtils, System.StrUtils, System.Math, Winapi.Windows,
  Winapi.Messages, Winapi.Imm, Winapi.DirectShow9, Winapi.ActiveX,
  Winapi.Direct3D9, Vcl.Graphics, Vcl.StdCtrls, Vcl.Controls, Vcl.Forms,
  Vcl.ComCtrls, Vcl.Grids, Vcl.Clipbrd, HGE, HGEBase, HGECanvas, HGETextures,
  HGEUtils, WIL, HUtil32, Winapi.D3DX9;
...
var
  ...
  TranFrame: Boolean = False;   //控制是否显示边框
...
procedure Register;
begin
  RegisterComponents('HGEGUI', [TDWinManager, TDControl, TDButton, TDGrid, TDWindow,
  TDCheckBox, TDUpDown, TDHooKKey, TDEdit, TDImageEdit,
  TDComboBox, TDListView, TDMemo, TDPopUpMemu, TDTreeView]);
end;
...
begin
  if Assigned(FOnDirectPaint) then
    ...
    with g_DXCanvas do begin
      TextOut(SurfaceX(Left) + (Width - TextWidth(Caption)) div 2, SurfaceY(Top) + (Height - TextHeight(Caption)) div 2, Caption, FColor);
    end;
  ...
  WinDc.Left := SurfaceX(Left);
  WinDc.Top := SurfaceY(Top);
  WinDc.Right := SurfaceX(Left + Width);
  WinDc.Bottom := SurfaceY(Top + Height);
  if TranFrame then
    g_DXCanvas.RoundRect(WinDc.Left, WinDc.Top, WinDc.Right, WinDc.Bottom, clLime);
end;
      ...
      case Key of
        Char(VK_BACK): begin
          ...
          if IsAllChars(Key) then begin
            if IsMBCSChar(Key){IsDBCSLeadByte(Ord(Key))} or bDoubleByte then begin
              bDoubleByte := True; //双字节
              Inc(KeyByteCount);
              FInputStr := FInputStr + key;
            end;
            if not bDoubleByte then begin //单字节
...
var
  WinDc: TRect;
begin
  if FItemList.Count > FItemCount then
  begin
    ...
    for i := 0 to FItem.Count - 1 do begin
      if FItem[i] <> '-' then begin
        if FItem.Objects[i] = nil then begin
          TextOut(Left + nLeft, Top + nHeight + (FHeightOffset - 12) div 2, FItem[i], FDFEnabledColor);
        end else
        if Integer(FItem.Objects[i]) = -1 then begin
          TextOut(Left + nLeft, Top + nHeight + (FHeightOffset - 12) div 2, FItem[i], FDFDownColor);
        end else begin
          if FItemIndex = i then begin
            FillRect(Left + FDlgWidth + 1, Top + nHeight, (FWidth - FDlgWidth * 2 - 2), FHeightOffset, $A062625A);
          end;
          TextOut(Left + nLeft, Top + nHeight + (FHeightOffset - 12) div 2, FItem[i], FDFColor);
        end;
...
begin
  nLen := FStartoffset;
  FEditString := '';
  for i := 0 to FEditTextList.Count - 1 do
  begin
...
begin
  ...
  nCount := 0;
  if (MaxLength = 0) or (Length(FEditString + LenStr) <= MaxLength) then
  begin
    for i := 0 to FEditTextList.Count - 1 do
    begin
      str := FEditTextList[i];
      if (str <> '') and (str[1] = #13) then
        Inc(nCount);
    end;
    if nCount >= FImageCount then
    begin
      Result := 4;
      exit;
    end;
...
begin
  ...
  begin
    ...
    begin
      ...
      else if (str <> '') and (str[1] = #10) then
        Inc(nCount);
    end;
    if nCount >= FItemCount then
    begin
      Result := 4;
      exit;
    end;
...
begin
  ...
  if (X < FStartoffset) and (FCaretPos > 0) then
  begin
    Dec(FCaretPos);
    RefEditSurfce;
  end
  else if (X > (Width - FStartoffset)) and (FCaretPos < FEditTextList.Count) then
  begin
    Inc(FCaretPos);
    RefEditSurfce;
  end;
  ...
  begin
    if FShowPos > 0 then
      ...
      OldLen := FStartoffset;
    for i := FShowPos to FEditTextList.Count - 1 do
    begin
...
begin
  ...
  else if (FCaretPos <= FShowPos) then
  begin
  ...
  begin
    ...
    begin
      ...
      begin
        ...
        if i = (FCaretPos - 1) then
          FShowLine := nLen;
        if (not boAdd) and (nLen > (Width - FStartoffset)) then
        begin
          boAdd := True;
          FOppShowPos := i - 1;
        end;
    ...
    begin
      ...
      begin
        ...
        if i = nStart then
          FStartLine := nLen;
        if (not boAdd) and (nLen < FStartoffset) then
        begin
          boAdd := True;
          FOppShowPos := i + 1;
        end;
      end;
      for i := ShowStrList.Count - 1 downto 0 do
      begin
      ...
      //Release;
      if (nLen > FStartoffset) and (boRef) then
      begin
        FShowLeft := True;
        FShowPos := 0;
        RefEditSurfce(False);
...
begin
  ...
  begin
    ...
    Result := True;
    for i := (nStop - 1) downto nStart do
    begin
      if i < 0 then
        break;
      if i >= FEditTextList.Count then
...
begin
  ...
  with g_DXCanvas do
  begin
    if (FStartX > -1) and (FStopX > -1) then
    begin
    ...
    if FEditImageList.Count > 0 then
    begin
      //FImageSurface.Fill(0);
      for i := 0 to FEditImageList.Count - 1 do
      begin
    ...
    if FCaretShow and (KeyControl = Self) then
    begin
      FrmIMEX := SurfaceX(FShowLine + left);
      if Height < 16 then
      begin
        RoundRect(SurfaceX(FShowLine + left), SurfaceY(Top), SurfaceX(left + FShowLine + 1), SurfaceY(top + Height), clWhite);
        FrmIMEY := SurfaceY(Top);
      end
      else
      begin
        RoundRect(SurfaceX(FShowLine + left), SurfaceY(Top + 2), SurfaceX(left + FShowLine + 1), SurfaceY(top + Height - 2), clWhite);
...
begin
  ...
  if str <> '' then
    ...
    begin
      ...
      else if Key = FImageChar then
      begin
        boImage := True;
        TempStr := '';
      end
      else if Key = FBeginChar then
      begin
        boItem := True;
        TempStr := '';
      end
...
begin
  ...
  begin
    ...
    if (ssCtrl in Shift) and (not Downed) and (Key = Word('X')) then
    begin
      if (FStartX > -1) and (FStopX > -1) and (FStartX <> FStopX) then
      begin
        Clipboard := TClipboard.Create;
    ...
    else if (ssCtrl in Shift) and (not Downed) and (Key = Word('C')) then
    begin
    ...
    else if (ssCtrl in Shift) and (not Downed) and (Key = Word('V')) then
    begin
    ...
    else if (ssCtrl in Shift) and (not Downed) and (Key = Word('A')) then
    begin
      SetFocus;
      Key := 0;
      Result := True;
    ...
    else if (ssShift in Shift) and (not Downed) then
    begin
      KeyDowned := True;
      if FStartX < 0 then
        FStartX := FCaretPos;
    ...
    case Key of
      ...
      VK_DELETE:
        begin
          boChange := ClearKey;
          if (not boChange) and (FEditTextList.Count > 0) then
          begin
            if FCaretPos < FEditTextList.Count then
...
begin
  Result := inherited MouseMove(Shift, X, Y);
  if Result and (MouseCaptureControl = self) then
  begin
    if Downed and (not KeyDowned) then
      MoveCaret(X - left, Y - top);
  end;
