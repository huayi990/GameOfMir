...
interface
uses
  System.Classes, System.SysUtils, Winapi.Windows, Winapi.Direct3D9,
  Vcl.Graphics, Vcl.Forms, HGE, HGEBase, HGETextures;
  ...
  for I := Low(Word) to High(Word) do begin
    ...
    if I = 0 then begin
      ...
      if not FiltrateChar(Char1, Char2) then begin
          ...
          Text := AnsiChar(Char1) + AnsiChar(Char2);
          wText := Text;
      ...
      if Char1 in [32..126] then Text := AnsiChar(Char1)
      else Text := AnsiChar(Char1) + AnsiChar(Char2);
    end;
